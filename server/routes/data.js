...
function importData(db, data) {
  ...
  if (data.sessions?.length) {
    const stmt = db.prepare(`INSERT OR IGNORE INTO sessions (id, date, duration, drills, notes, intention, session_type, position, quick_rating, body_check, shooting, passing, fitness, delivery, attacking, reflection) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
